...
const ProfileScreen = ({ navigation, route }: { navigation: any; route?: any }) => {
  ...
  useEffect(() => {
    if (updatedPostData && productionProfile) {
      console.log('📝 Updating post with new data:', updatedPostData);
      
      // Update the post in productionProfile.posts
      const updatedPosts = productionProfile.posts.map(post => 
        post.id === updatedPostData.id 
          ? { ...post, like_count: updatedPostData.like_count, user_liked: updatedPostData.user_liked }
          : post
      );
      
      setProductionProfile(prev => prev ? {
        ...prev,
        posts: updatedPosts
      } : null);
  ...
  }, [updatedPostData, productionProfile, navigation]);
  ...
  const handlePostPress = (post: any) => {
    ...
    navigation.navigate('PostDetail', { 
      postId: post.id,
      post: post as any,
      sourceScreen: 'Profile', // Tell PostDetail where to send updates
      userProfile: {
  ...
  return (
    ...
    >
      ...
      <View style={styles.profileHeader}>
        ...
        {/* Agentic Manager Button */}
        <TouchableOpacity 
          style={styles.agenticManagerButton}
          onPress={() => navigation.navigate('AgenticManager')}
        >
          <LinearGradient
            colors={['#3B82F6', '#2563EB', '#1D4ED8']}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={styles.agenticManagerGradient}
          >
            <Ionicons name="flash" size={18} color="#FFFFFF" />
            <Text style={styles.agenticManagerText}>Agentic Manager</Text>
            <View style={styles.agenticManagerBadge}>
              <Text style={styles.agenticManagerBadgeText}>AI</Text>
            </View>
          </LinearGradient>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.quickActionsRow}>
        ...
        {activeProfileType === 'artist' && (
          <>
            ...
            <TouchableOpacity 
              style={styles.quickActionBtn}
              onPress={() => navigation.getParent()?.navigate('Investment')}
            >
...
const styles = StyleSheet.create({
  ...
  agenticManagerButton: {
    marginTop: 10,
    borderRadius: 10,
    overflow: 'hidden',
  },
  agenticManagerGradient: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
    paddingVertical: 10,
